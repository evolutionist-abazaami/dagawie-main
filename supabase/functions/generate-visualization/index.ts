...
serve(async (req) => {
  ...
  try {
    ...
    const cacheKey = `${visualizationType}_${region}_${eventType}_${changePercent}_${JSON.stringify(spectralIndices || {}).substring(0, 100)}`;
    const cacheKeyHash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(cacheKey));
    const cacheKeyHex = Array.from(new Uint8Array(cacheKeyHash)).map(b => b.toString(16).padStart(2, '0')).join('');
    ...
    try {
      ...
      if (cachedResult.data) {
        console.log(`Cache hit for ${visualizationType} - returning cached image`);
        return new Response(
          JSON.stringify({ 
            success: true,
            imageUrl: cachedResult.data.image_url,
            description: "Visualization retrieved from cache",
            visualizationType,
            cached: true
          }),
      ...
      console.log('Cache not available or table not created yet:', cacheError.message);
...
Major transitions: ${changeDetection.major_changes?.map((c: unknown) => (c as { type: string }).type).join(', ')}` : `Change: ${Math.abs(changePercent)}%`}
...
${classificationResults.classes?.slice(0, 8).map((c: { name?: string; area_percent?: number }) => `- ${c.name}: ${c.area_percent?.toFixed(1)}%`).join('\n')}
...
${changeDetection.change_matrix
  ...
  .map(
    (c: {
      ...
      `- ${c.from_class} → ${c.to_class}: ${c.area_km2?.toFixed(1)} km² (${c.percent?.toFixed(
        1,
      )}%)`,
  )
  .join("\n")}`
...
HOTSPOTS: ${changeDetection.change_hotspots.slice(0, 3).map((h: { location?: string }) => h.location).join(', ')}` : ''}
...
Major changes: ${changeDetection.major_changes?.slice(0, 2).map((c: { type?: string }) => c.type).join(', ')}` : ''}
...
${changeDetection?.change_hotspots ? `Highlight hotspots: ${changeDetection.change_hotspots.slice(0, 3).map((h: { location?: string }) => h.location).join(', ')}` : ''}
...
${changeDetection?.change_hotspots ? `
Priority Hotspots: ${changeDetection.change_hotspots.slice(0, 4).map((h: { location?: string }) => h.location).join(', ')}` : ''}
...
Driver Breakdown:
${changeDetection.change_drivers.slice(0, 5).map((d: { driver?: string; contribution_percent?: number }) => `- ${d.driver}: ${d.contribution_percent}%`).join('\n')}` : `
Estimated Drivers:
        ...
        await supabase
          ...
          .upsert({
            ...
            parameters: {
              changePercent,
              severity,
              spectralIndices: spectralIndices ? JSON.stringify(spectralIndices).substring(0, 500) : null
            }
          });
        console.log(`Cached ${visualizationType} visualization for future use`);
      } catch (cacheError) {
        console.log('Failed to cache visualization (table may not exist yet):', cacheError.message);
