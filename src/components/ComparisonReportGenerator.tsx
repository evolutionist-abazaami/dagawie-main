...
const ComparisonReportGenerator = ({ comparisonResult }: ComparisonReportGeneratorProps) => {
  ...
  const generateComparisonPDF = async () => {
    ...
    try {
      ...
      const reportId = `DG-CMP-${reportDate.getFullYear()}${String(reportDate.getMonth() + 1).padStart(2, '0')}${String(reportDate.getDate()).padStart(2, '0')}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
      ...
      const checkPageBreak = (requiredSpace: number): void => {
        if (yPos > pageHeight - requiredSpace) {
          ...
          pdf.text(`Dagawie Comparison Report ${reportId}`, margin, 10);
      ...
      pdf.setFontSize(7);
      pdf.setFont("helvetica", "normal");
      pdf.text("Dagawie Environmental Intelligence Platform", margin, pageHeight - 9);
      pdf.text(`Generated: ${reportDate.toISOString().split('T')[0]}`, margin, pageHeight - 5);
      
      pdf.setFont("helvetica", "bold");
      pdf.text(formatSiteHost(BRAND.siteUrl), pageWidth - margin - 28, pageHeight - 7);

      // Save PDF
      const filename = `Dagawie_Comparison_${comparisonResult.location.replace(/[^a-zA-Z0-9]/g, "_").substring(0, 20)}_${reportDate.toISOString().split("T")[0]}.pdf`;
      pdf.save(filename);
