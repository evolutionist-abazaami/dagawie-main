...
import { BRAND, formatSiteHost } from "@/lib/branding";
...
interface AnalysisData {
  ...
  confidence?: number;
  findings?: Array<{ num: string; title: string; detail: string }>;
  recommendations?: Array<{ num: string; title: string; detail: string }>;
}
...
const ReportGenerator = ({ analysisData, eventType, region }: ReportGeneratorProps) => {
  ...
  const [selectedAdvancedViz, setSelectedAdvancedViz] = useState<string[]>(['risk_zones']);
  ...
  const generateVisualization = async (type: string): Promise<string | null> => {
    try {
      ...
      if (error.name === 'AbortError') {
        console.warn(`Visualization generation timed out for ${type}`);
      } else {
  ...
  const loadImageAsBase64 = async (dataUrl: string, retries = 2): Promise<{ data: string; width: number; height: number } | null> => {
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (!response.ok) {
          ...
          console.warn("Failed to fetch image after retries:", dataUrl, response.status);
          return null;
        }
        ...
        const result = await new Promise<{ data: string; width: number; height: number } | null>((resolve) => {
          img.onload = () => {
            try {
          ...
          img.onerror = () => {
            console.warn("Failed to load image blob:", dataUrl);
            resolve(null);
          };
        ...
        console.warn(`Failed to fetch image (attempt ${attempt + 1}):`, dataUrl, e);
        if (attempt < retries) continue;
      }
    }
    return null;
  ...
  const generateVisualizationsParallel = async (visualizations: Array<{type: string, step: string}>): Promise<Record<string, string | null>> => {
    const results: Record<string, string | null> = {};
    const total = visualizations.length;
    let completed = 0;
    
    // Start all requests in parallel
    const promises = visualizations.map(async ({type, step}) => {
      setGenerationStep(`${step} (${++completed}/${total})`);
      const image = await generateVisualization(type);
      results[type] = image;
      return image;
    });
  ...
  const generatePDFReport = async () => {
    ...
    try {
      ...
      if (includeImages) {
        // Prepare all visualizations to generate in parallel
        const visualizationsToGenerate: Array<{type: string, step: string}> = [];

        // Always generate all core satellite imagery for reports
        visualizationsToGenerate.push(
          { type: "landsat_truecolor", step: "Generating Landsat true-color satellite imagery..." },
          { type: "landsat_falsecolor", step: "Generating false-color composite imagery..." },
          { type: "ndvi_map", step: "Generating NDVI vegetation analysis map..." },
          { type: "classification", step: "Generating land cover classification..." },
          { type: "change_detection", step: "Generating change detection analysis..." },
          { type: "chart", step: "Generating trend analysis chart..." }
        );

        // Add all selected advanced visualizations
        selectedAdvancedViz.forEach(vizId => {
          const vizInfo = ADVANCED_VIZ_TYPES.find(v => v.id === vizId);
          if (vizInfo) {
            visualizationsToGenerate.push({
              type: vizId,
              step: `Generating ${vizInfo.label}...`
            });
          }
        });

        // Generate all visualizations in parallel
        setGenerationStep("Generating visualizations in parallel...");
        const visualizationResults = await generateVisualizationsParallel(visualizationsToGenerate);
        
        // Extract results - use consistent key names
        trueColorImage = visualizationResults["landsat_truecolor"];
        falseColorImage = visualizationResults["landsat_falsecolor"];
        ndviImage = visualizationResults["ndvi_map"];
        classificationImage = visualizationResults["classification"];
        changeDetectionImage = visualizationResults["change_detection"];
        chartImage = visualizationResults["chart"];
        ...
        console.log("Generated images:", {
          ...
          changeDetectionImage: !!changeDetectionImage,
          chartImage: !!chartImage
        });
        
        // Extract advanced images
        selectedAdvancedViz.forEach(vizId => {
          advancedImages[vizId] = visualizationResults[vizId];
        });
      }

      // Load all images as base64 for PDF inclusion
      setGenerationStep("Loading images for PDF...");
      const loadedImages: Record<string, { data: string; width: number; height: number } | null> = {};
      
      const imagesToLoad = [];
      if (trueColorImage) imagesToLoad.push({ key: 'trueColor', url: trueColorImage });
      if (falseColorImage) imagesToLoad.push({ key: 'falseColor', url: falseColorImage });
      if (ndviImage) imagesToLoad.push({ key: 'ndvi', url: ndviImage });
      if (classificationImage) imagesToLoad.push({ key: 'classification', url: classificationImage });
      if (changeDetectionImage) imagesToLoad.push({ key: 'changeDetection', url: changeDetectionImage });
      if (chartImage) imagesToLoad.push({ key: 'chart', url: chartImage });
      
      // Load advanced images
      Object.entries(advancedImages).forEach(([key, url]) => {
        if (url) imagesToLoad.push({ key: `advanced_${key}`, url });
      });

      // Load images in parallel
      const loadPromises = imagesToLoad.map(async ({ key, url }) => {
        const loaded = await loadImageAsBase64(url);
        loadedImages[key] = loaded;
        return loaded;
      });
      ...
      const reportId = `DG-${reportDate.getFullYear()}${String(reportDate.getMonth() + 1).padStart(2, '0')}${String(reportDate.getDate()).padStart(2, '0')}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
      ...
      const addPageHeader = (): void => {
        ...
        pdf.setFont("helvetica", "normal");
        pdf.text(BRAND.product, margin, 12);
      ...
      const addPageFooter = (pageNum: number, totalPages: number): void => {
        ...
        pdf.text(`CONFIDENTIAL - For authorized use only | © 2026 ${BRAND.name}`, pageWidth / 2, pageHeight - 8, { align: "center" });
      };
      ...
      pdf.setFontSize(28);
      pdf.setFont("helvetica", "bold");
      pdf.text("DAGAWIE INTELLIGENCE", margin, 40);
      
      pdf.setFontSize(12);
      pdf.setFont("helvetica", "normal");
      pdf.text("Geospatial Environmental Intelligence Platform", margin, 52);
      ...
      pdf.setTextColor(255, 255, 255);
      pdf.setFontSize(9);
      pdf.text(BRAND.product, margin, pageHeight - 15);
      pdf.text(`${formatSiteHost(BRAND.siteUrl)} | ${BRAND.supportEmail}`, margin, pageHeight - 8);
      ...
      findings.slice(0, 4).forEach((finding: { num?: string; title?: string; detail?: string; description?: string } | string, index: number) => {
        checkPageBreak(20);
      ...
      if (!isSimple) {
        ...
        if (loadedImages.trueColor) {
          try {
            const img = loadedImages.trueColor;
            pdf.addImage(img.data, "PNG", margin + 5, yPos + 5, imgWidth, imgHeight);
          } catch (e) {
        ...
        if (loadedImages.falseColor) {
          try {
            const img = loadedImages.falseColor;
            pdf.addImage(img.data, "PNG", margin + 3, yPos + 3, gridImageWidth - 6, gridImageHeight);
          } catch (e) {
        ...
        if (loadedImages.ndvi) {
          try {
            const img = loadedImages.ndvi;
            pdf.addImage(img.data, "PNG", ndviX + 3, yPos + 3, gridImageWidth - 6, gridImageHeight);
          } catch (e) {
      ...
      if (loadedImages.chart) {
        try {
          const img = loadedImages.chart;
          pdf.addImage(img.data, "PNG", margin + 5, yPos + 5, chartImgWidth, chartImgHeight);
        } catch (e) {
      ...
      if (!isSimple) {
        ...
        yPos += 8;
        yPos = addSubsectionTitle("Literature Comparison", yPos);
        
        const literatureText = "Dagawie Intelligence results are compared with established geospatial analysis platforms and academic research. Methodologically, our AI-driven approach using Sentinel-2 imagery aligns with studies by Hansen et al. (2013) for deforestation monitoring and Pekel et al. (2016) for global surface water mapping. Unlike traditional GIS platforms such as ArcGIS Pro or QGIS, Dagawie Intelligence provides automated change detection with integrated AI interpretation, offering advantages in processing speed and accessibility for non-expert users.";
        
        const literatureLines = pdf.splitTextToSize(literatureText, contentWidth);
        pdf.text(literatureLines, margin, yPos);
        yPos += literatureLines.length * 5 + 5;

        pdf.setFontSize(10);
        pdf.setFont("helvetica", "bold");
        pdf.text("Key Comparative Platforms:", margin, yPos);
        yPos += 8;
        
        const platforms = [
          "• Google Earth Engine: Cloud-based processing, similar satellite data sources",
          "• ESA Climate Change Initiative: Global land cover products, peer-reviewed validation",
          "• NASA SERVIR: Applied science for decision-making, regional focus",
          "• UNEP-WCMC: Conservation monitoring, policy-oriented outputs"
        ];

        platforms.forEach((platform) => {
          pdf.text(platform, margin + 5, yPos);
          yPos += 6;
        });
        ...
        pdf.setFontSize(10);
        pdf.setFont("helvetica", "bold");
        pdf.text("Academic References (APA Format):", margin, yPos);
        yPos += 8;
        ...
        references.forEach((ref) => {
          const refLines = pdf.splitTextToSize(ref, contentWidth - 10);
          pdf.text(refLines, margin + 5, yPos);
          yPos += refLines.length * 4 + 2;
        });
      }
      ...
      if (includeImages && (classificationImage || changeDetectionImage)) {
        ...
        if (loadedImages.classification) {
          // try {
            yPos = addSubsectionTitle("Land Cover Classification Map", yPos);
            pdf.setFillColor(243, 244, 246);
            pdf.roundedRect(margin, yPos, contentWidth, 70, 3, 3, "F");
            const img = loadedImages.classification;
            pdf.addImage(img.data, "PNG", margin + 5, yPos + 5, contentWidth - 10, 60);
            yPos += 75;
        ...
        if (loadedImages.changeDetection) {
          try {
            checkPageBreak(90);
            yPos = addSubsectionTitle("Change Detection Analysis", yPos);
            pdf.setFillColor(243, 244, 246);
            pdf.roundedRect(margin, yPos, contentWidth, 70, 3, 3, "F");
            const img = loadedImages.changeDetection;
            pdf.addImage(img.data, "PNG", margin + 5, yPos + 5, contentWidth - 10, 60);
            yPos += 75;
        ...
        // Add advanced visualizations if any were generated
        Object.entries(loadedImages).forEach(([key, imgData]) => {
          if (key.startsWith('advanced_') && imgData) {
            checkPageBreak(90);
            try {
              const vizType = key.replace('advanced_', '');
              const vizInfo = ADVANCED_VIZ_TYPES.find(v => v.id === vizType);
              const title = vizInfo ? vizInfo.label : vizType.replace(/_/g, ' ').toUpperCase();
              
              yPos = addSubsectionTitle(`${title} Analysis`, yPos);
              pdf.setFillColor(243, 244, 246);
              pdf.roundedRect(margin, yPos, contentWidth, 70, 3, 3, "F");
              pdf.addImage(imgData.data, "PNG", margin + 5, yPos + 5, contentWidth - 10, 60);
              yPos += 75;
              
              pdf.setFontSize(8);
              pdf.setTextColor(107, 114, 128);
              pdf.setFont("helvetica", "italic");
              pdf.text(`${title} visualization providing specialized environmental analysis.`, margin, yPos);
              yPos += 12;
            } catch (e) {
              console.warn(`Could not add advanced image ${key}:`, e);
            }
          }
        });
      }
      ...
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(9);
      const disclaimer = "Dagawie Intelligence provides geographic analysis tools for research and decision-support purposes only. Outputs are based on available data and computational models and may contain inaccuracies. Results should not be considered legal, engineering, or regulatory determinations. Users are responsible for independently verifying all findings before making critical decisions.";
      ...
      pdf.setTextColor(255, 255, 255);
      pdf.setFontSize(9);
      pdf.text(BRAND.product, margin, pageHeight - 10);
      pdf.text(`${formatSiteHost(BRAND.siteUrl)} | ${BRAND.supportEmail}`, pageWidth / 2 - 30, pageHeight - 10);
      pdf.text(`© ${new Date().getFullYear()} ${BRAND.name}. All Rights Reserved`, pageWidth - margin - 55, pageHeight - 10);
      ...
      // Save PDF
      const filename = `Dagawie_${reportType === "professional" ? "Professional" : "Summary"}_Report_${reportId}.pdf`;
      pdf.save(filename);
        ...
        <div className="flex items-center justify-between p-3 bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 rounded-lg">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Generate report 3x faster with fewer visualizations</p>
            </div>
          </div>
          <Button
          ...
          >
            {fastMode ? <Check className="h-4 w-4" /> : "Off"}
          </Button>
        </div>
      )}

      {/* Advanced Visualization Types - Only show for Professional reports with images and not in fast mode */}
      {reportType === "professional" && includeImages && !fastMode && (
                  ...
                  <ScrollArea className="h-[400px] rounded-md border p-4">
                    <div className="space-y-4">
                      <div className="p-3 bg-primary/10 rounded-lg">
                        ...
                        <p className="text-sm text-muted-foreground">Dagawie Intelligence branding, report metadata, classification badge</p>
                      </div>
