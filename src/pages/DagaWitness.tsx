...
import MapLibreMap, { HeatmapLayerType, BaseMapType } from "@/components/MapLibreMap";
...
interface MultiEventAnalysis {
  events: MultiEvent[];
  combined_impact?: string;
}

interface DagaWitnessResults {
  isMultiEvent?: boolean;
  eventType?: string;
  severity?: 'low' | 'medium' | 'high' | 'critical' | string;
  multiEventAnalysis?: MultiEventAnalysis;
  landsatInfo?: {
    sensor?: string;
  ...
  spectralIndices?: Record<string, unknown>;
  classificationResults?: Record<string, unknown>;
  changeDetection?: Record<string, unknown>;
}
...
const DagaWitness = () => {
  console.log("DagaWitness component rendering");
  ...
  const [results, setResults] = useState<DagaWitnessResults | null>(null);
    ...
    <div className="h-[calc(100vh-73px)] flex flex-col bg-background">
      ...
      <div className="w-full lg:w-[420px] bg-card lg:border-l border-b lg:border-b-0 border-border overflow-y-auto order-1 lg:order-2">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
          <TabsList className="w-full justify-start rounded-none border-b border-border p-0 h-auto bg-transparent overflow-x-auto whitespace-nowrap">
            <TabsTrigger 
              value="search" 
              className="flex-none min-w-[80px] rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent py-2 px-2 text-xs sm:text-sm whitespace-nowrap"
            >
              ...
              <div className="space-y-4 pt-4 border-t border-border">
                ...
                {results.multiEventAnalysis?.events && (
                  <div className="space-y-2">
                    ...
                    {results.multiEventAnalysis.events.map((event: MultiEvent, i: number) => (
                      <div key={i} className="p-2 bg-muted/30 rounded-md flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Badge
                            variant={event.severity === 'critical' ? 'destructive' : event.severity === 'high' ? 'destructive' : 'secondary'}
                            className="text-xs"
                          >
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <GISExportButton
                    ...
                    filename={`dagawie-${eventType}-${region?.replace(/[^a-zA-Z0-9]/g, "_") || "analysis"}`}
                  />
                </div>
              </div>
            )}
          ...
          <TabsContent value="upload" className="flex-1 p-4 md:p-6 mt-0 overflow-y-auto">
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-800">
                <strong>Advanced Data Integration</strong> - Optional enhancement feature. Upload your own geospatial data files (Shapefiles or GeoJSON) to combine with satellite analysis for more comprehensive insights.
              </p>
            </div>
            <FileUploadAnalysis />
          </TabsContent>
