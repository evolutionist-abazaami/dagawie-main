    ...
    <div className="min-h-[calc(100vh-73px)] bg-background p-4 md:p-6">
      <div className="max-w-7xl mx-auto space-y-4 md:space-y-6">
        ...
        <Tabs defaultValue="analyses" className="space-y-4">
          <TabsList>
            <TabsTrigger value="analyses">DagaWitness Analyses</TabsTrigger>
            <TabsTrigger value="searches">DagaSearch Queries</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="analyses" className="space-y-4">
            <div className="flex flex-col sm:flex-row justify-end gap-2">
              <Button
                variant="outline"
                size="sm"
                onClick={() => exportToCSV(analysisResults, "dagawitness_analyses")}
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => exportToJSON(analysisResults, "dagawitness_analyses")}
                className="w-full sm:w-auto"
              >
                <Download className="h-4 w-4 mr-2" />
                Export JSON
            ...
            <div className="space-y-3">
              {analysisResults.length === 0 ? (
                <Card className="p-8 text-center">
                  ...
                  <Button variant="link" onClick={() => navigate("/dagawitness")}>
                    Start analyzing
                  </Button>
                </Card>
              ) : (
          ...
          <TabsContent value="searches" className="space-y-4">
            <div className="flex flex-col sm:flex-row justify-end gap-2">
              <Button
                variant="outline"
                size="sm"
                onClick={() => exportToCSV(searchQueries, "dagasearch_queries")}
                className="w-full sm:w-auto"
              >
                <Download className="h-4 w-4 mr-2" />
                Export CSV
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => exportToJSON(searchQueries, "dagasearch_queries")}
                className="w-full sm:w-auto"
              >
                <Download className="h-4 w-4 mr-2" />
                Export JSON
            ...
            <div className="space-y-3">
              {searchQueries.length === 0 ? (
                <Card className="p-8 text-center">
                  ...
                  <Button variant="link" onClick={() => navigate("/dagasearch")}>
                    Start searching
                  </Button>
                </Card>
              ) : (
